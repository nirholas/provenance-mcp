// Real HTTP access to the three.ws agent-actions ledger. No mocks, no fixtures —
// every call is a live request to THREE_WS_BASE. The ledger authenticates every
// request, so a Bearer token (THREE_WS_TOKEN) is attached automatically when
// present. Errors are normalized into a single shape so tool handlers can
// surface a clean message + status to the MCP client.

import { THREE_WS_BASE, HTTP_TIMEOUT_MS, USER_AGENT, authToken } from '../config.js';

/**
 * Call a three.ws HTTP endpoint and return its parsed JSON body.
 *
 * The Authorization header is added from THREE_WS_TOKEN when configured. The
 * ledger rejects unauthenticated calls (HTTP 401), so a missing token surfaces
 * as a normal upstream error the handler can explain.
 *
 * @param {string} path  Endpoint path beginning with `/` (e.g. `/api/agent-actions`).
